...
public class CardSlidePanel extends ViewGroup {
    ...
    // TODO test
    public void updateCardViewImage(int index, String imgPath) {
        CardItemView itemView = viewList.get(0);
        CardDataItem dataItem = dataList.get(index);
        dataItem.imagePath = imgPath;
        itemView.fillData(dataItem);
        itemView.setVisibility(VISIBLE);

        if (null != cardSwitchListener) {
            cardSwitchListener.onShow(itemView, index);
        }
    }
