...
public class BucketGridAdapter extends ArrayAdapter<BucketEntry> {
    ...
    private final static int DNMTR = 3;  // Grid一行显示多少个
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            imageParams.width = mWidth / DNMTR;
            imageParams.height = mWidth / DNMTR;
        ...
        if (mIsFromVideo) {
            new VideoLoadAsync(bucketVideoFragment, holder.imageView, false, mWidth / DNMTR).executeOnExecutor(MediaAsync.THREAD_POOL_EXECUTOR, mBucketEntryList.get(position).bucketUrl);

        } else {
            ImageLoadAsync loadAsync = new ImageLoadAsync(mContext, holder.imageView, mWidth / DNMTR);
