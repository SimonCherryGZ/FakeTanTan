...
public class HomeScreenMediaChooser extends AppCompatActivity implements ImageFragment.OnImageSelectedListener, VideoFragment.OnVideoSelectedListener {
    ...
    public void onImageSelected(int count) {
        if (tabLayout.getChildCount() > 1 &&  tabLayout.getTabAt(1) != null) {
            if (count != 0) {
        ...
        } else if (tabLayout.getChildCount() > 0 &&  tabLayout.getTabAt(0) != null) {
            if (count != 0) {
                tabLayout.getTabAt(0).setText(getResources().getString(R.string.images_tab) + "  " + count);

            } else {
                tabLayout.getTabAt(0).setText(getResources().getString(R.string.image));
            }
        }
    }
