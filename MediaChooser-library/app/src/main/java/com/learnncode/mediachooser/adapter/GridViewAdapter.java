    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mIsFromVideo) {
            if (!mGalleryModelList.get(position).isCallStarted) {
                ...
                new VideoLoadAsync(videoFragment, holder.imageView, false, mWidth / DNMTR).executeOnExecutor(MediaAsync.THREAD_POOL_EXECUTOR, mGalleryModelList.get(position).url);
            }
        } else {
            if (!mGalleryModelList.get(position).isCallStarted) {
                mGalleryModelList.get(position).isCallStarted = true;
